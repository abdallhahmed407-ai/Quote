import { esc, orderedList, page, partyField, type RenderContext } from './render-utils';
...
export function renderTermsPages(context: RenderContext): string {
  const parties = `
    <h1 class="sec-title">الشروط والأحكام</h1><div class="parties-row">
      <div class="party-card"><h3>الطرف الأول (أجور)</h3>
        ${partyField('الاسم', 'شركة الرائدة للموارد البشرية — أجور')}
        ${partyField('السجل التجاري', '—')}${partyField('الرقم الضريبي', '—')}
        ${partyField('العنوان', 'مبنى 8758، حي العليا، مكتب 309، الرمز البريدي 12214، الدور الثالث')}</div>
      <div class="party-card"><h3>الطرف الثاني (العميل)</h3>
        ...
        ${partyField('العنوان', esc(context.customerAddress), true)}</div>
    </div><div class="terms-sec">الالتزامات</div><p class="terms-subsec">الطرف الأول:</p>
    ${orderedList(firstPartyObligations)}`;

  const obligations = `
    <h1 class="sec-title">الشروط والأحكام</h1><div class="terms-sec">الالتزامات</div>
    <p class="terms-subsec">الطرف الثاني:</p>${orderedList(secondPartyObligations)}
    <div class="terms-sec">الالتزامات المالية</div>${orderedList(financialObligations)}`;

  const signatures = `
    <h1 class="sec-title">الشروط والأحكام</h1><div class="terms-sec">أحكام عامة</div>
    <p class="terms-subsec">الفصل في النزاعات:</p>${orderedList(disputeTerms)}
    <p class="terms-subsec">إنهاء الخدمة والاسترداد:</p>
    ...
    <div class="sigs-row">
      <div class="sig-block"><h3>توقيع الطرف الأول</h3>
        ...
        <div class="sig-line-row"><span class="sl-label">التوقيع:</span><span class="sl-dots"></span></div></div>
      <div class="sig-block"><h3>توقيع الطرف الثاني</h3>
        <div class="sig-line-row"><span class="sl-label">الاسم:</span><span class="signature-value">${esc(context.contactName)}</span></div>
        <div class="sig-line-row"><span class="sl-label">التاريخ:</span><span class="signature-value ltr">${esc(context.createdDate)}</span></div>
        <div class="sig-line-row"><span class="sl-label">التوقيع:</span><span class="sl-dots"></span></div></div>
    </div>`;

  return [page('الشروط والأحكام', parties, 'terms-page'),
    page('الشروط والأحكام', obligations, 'terms-page'),
    page('الشروط والأحكام', signatures, 'terms-page')].join('');
}
